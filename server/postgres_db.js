...
export class PostgresCoordinator {
      ...
      for (let v of streamInfos) {
        await client.query(
          ...
          [
            ...
            v.title,
            v.thumbnail,
            v.streamStartTime?.getTime?.() || null,
            v.isMembersOnly,
            v.streamType,
            ts,
          ]
  ...
  async setConfig(key, value) {
    ...
    try {
      await this.connection.query(
        `INSERT INTO config VALUES ($1, $2) ON CONFLICT (name) DO UPDATE SET val=excluded.val`,
        [key, value]
      );
    } catch (e) {
  ...
  async getConfig(key) {
    ...
    try {
      res = await this.connection.query(
        `SELECT val FROM config WHERE name = $1 LIMIT 1`,
        [key]
      );
    } catch (e) {
