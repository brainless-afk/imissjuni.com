...
export class TextCountdown extends Component {
  ...
  componentDidMount() {
    this.timer = setInterval(
      () => this.setState({ delta: this.props.to - Date.now() }),
      1000
    );
  }
  ...
  render() {
    const isNow = this.isCloseToNow(this.state.delta),
      isNegative = this.state.delta < 0,
      effectiveDelta = Math.abs(this.state.delta);
    if (isNow) {
