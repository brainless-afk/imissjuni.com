  ...
  return (
    <div className={`${styles.videoBox}`}>
      <div className={styles.vstack}>
          ...
          <p className={`${styles.videoBoxCaption}`}>
            ...
            {props.showCountdown && props.info.startTime ? (
              <span className={styles.countdown}>
                <TextCountdown
                  to={props.info.startTime}
                  formatStrings={dict.Countdowns.VideoBox}
                />
              </span>
            ) : null}
          </p>
        ...
        <p>
          <a href={props.info.link}>{props.info.title}</a>
        </p>
        {props.info.isMembersOnly ? (
          <p>{lang.VideoBox.MembersOnlySubtext}</p>
        ) : null}
      </div>
      {props.info.thumbnail ? (
        <img
          src={props.info.thumbnail}
          alt={lang.VideoBox.ThumbnailAltText}
          width={120}
        />
      ) : null}
    </div>
