...
import {
  LangContext,
  useLocalizationForRootComponentsOnly,
...
export async function getServerSideProps({ req, res, query }) {
  ...
  if (process.env.USE_DUMMY_DATA !== "true") {
    res.setHeader(
      "Cache-Control",
      "max-age=0, s-maxage=90, stale-while-revalidate=180"
    );
  }
  ...
  switch (status) {
    ...
    case STREAM_STATUS.STARTING_SOON:
      return lang.formatString(
        lang.Main.Embed.TextStartingSoon,
        streamInfo.title
      );
    default:
      return lang.formatString(
        lang.Main.Embed.TextStreamQueued,
        streamInfo.title
      );
  }
}
    ...
    return (
      <div className={`${styles.streamInfo} ${boxExtraClass}`}>
        <VideoBox
          caption={text}
          info={props.info}
          showCountdown={props.status != STREAM_STATUS.LIVE}
        />
      </div>
    );
  } else {
      ...
      <div className={styles.streamInfo}>
        <div className={styles.vstack}>
          <p className={styles.videoBoxCaption}>
            {lang.VideoBox.NoStreamDummyStatus}
          </p>
          <p>
            <b>{lang.VideoBox.NoStreamDummyTitle}</b>
          </p>
        </div>
      </div>
    );
...
function NoStreamLayout(props) {
  ...
  return (
    <div>
      ...
      <PastStreamCounter />
      <p>
        <Link href="/reps">{lang.Main.RandomVodLink}</Link>
      </p>
      <CommonFooter channelLink={props.channelLink} />
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className={`${styles.streamInfo} ${styles.streamInfoError}`}>
        <p>
          {lang.formatString(
            lang.Main.ErrorOccurred,
            <a href={props.channelLink}>{lang.Main.ErrorMessageChannelLink}</a>
          )}
        </p>
      </div>
      <PastStreamCounter />
...
function DebugBar(props) {
  return (
    <div>
      ...
      <button
        onClick={() => {
          props.setQueryString("?mock=live");
        }}
      >
        Live
      </button>
      <button
        onClick={() => {
          props.setQueryString("?mock=soon");
        }}
      >
        Soon
      </button>
      <button
        onClick={() => {
          props.setQueryString("?mock=farout");
        }}
      >
        Early Frame
      </button>
      <button
        onClick={() => {
          props.setQueryString("?mock=nostream");
        }}
      >
        No Stream
      </button>
      <button
        onClick={() => {
          props.setQueryString("?mock=error");
        }}
      >
        Error
      </button>
      <button
        onClick={() => {
          props.mutate(API_ROUTES.STREAM_INFO);
        }}
      >
        (Refresh Now)
      </button>
...
export default function Home(props) {
  ...
  const { data } = useSWR(
    API_ROUTES.STREAM_INFO,
    (url) => refreshStreamInfo(url, debugMockType),
    {
      fallbackData: {
  ...
  return (
    <LangContext.Provider value={useLocalizationForRootComponentsOnly()}>
      <div className={styles.site}>
        ...
        {layout}
        {props.showDebugBar ? (
          <DebugBar mutate={mutate} setQueryString={setDebugMockType} />
        ) : null}
      </div>
